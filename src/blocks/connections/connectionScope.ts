...
import { canBlockContainChildren, isBlockShape, type BlockShape } from '../blockModel'
...
export function pairBlockFaces(
	...
	const live = (block: BlockShape) => (
		!(options.requireLive ?? true) || canBlockContainChildren(block.props.view)
	)
...
export function anchorFaceForScope(
	...
	if (scopeId === shape.id) return canBlockContainChildren(shape.props.view) ? 'inner' : null
