  ...
  return attributes.map((attribute) => {
    ...
    return (
      ...
      >
        ...
        >
          <b>{attribute.name}</b>
          {attribute.type ? <><span className="TypeAttributeRegion-punctuation">: </span><span className="TypeAttributeRegion-type">{attribute.type}</span></> : null}
          {attribute.value ? <><span className="TypeAttributeRegion-punctuation"> = </span><span>{attribute.value}</span></> : null}
        </button>
...
/**
 * The Type's source is deliberately a normal multiline text transaction. The
 * compact tree is faster to read, but making each pretty row independently
 * writable would fork the one source body and make paste/Enter non-native.
 */
