...
/**
 * Block, Branch, Pill and Port share one toolbar slot, so the slot has to remember
...
function rememberSystemTools(tools: TLUiToolsContextType): TLUiToolsContextType {
  const next: TLUiToolsContextType = { ...tools }
  for (const id of ['block', 'branch', 'loop', 'pill', 'floating-port'] as const satisfies readonly SystemFamilyTool[]) {
...
export const SYSTEMSKETCH_TOOLBAR_OVERRIDES: TLUiOverrides = {
  tools: (editor, tools) =>
    rememberSystemTools(withFloatingPortTool(editor, withLoopTool(editor,
