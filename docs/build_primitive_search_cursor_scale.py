...
def main() -> None:
    ...
    scale_check = "at 160% interface scale, the search target and panel still open beside the pointer in canvas pixels"
    if measurements["interfaceScale"] != 160 or scale_check not in checks:
        raise SystemExit("refusing to build without the verified 160% browser run")
