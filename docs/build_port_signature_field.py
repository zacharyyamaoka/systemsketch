...
<p><b>Judged, round 10 — PASS</b>, the loop's cap. A tenth auditor reverted the guard in a scratch tree and the journey went red on the new step alone (with those three assertions neutralised, all 21 stayed green — the only protection is the new one); proved the stub shadows the prototype method and the tracked read re-runs on the remount, so the step cannot pass vacuously; confirmed the restored method is the identical prototype function and the later steps run portalled; drove the pan gate at zoom 2, 1, 0.5 and 0.25 (residual 0 px at each; the old gate went red at 0.25 on a correct lane); and rebuilt the page from the tree — the only diff is the commit stamp. Two low, test-side risks stand documented: the step expects <code>w + 12</code> literally, so changing the port-label inset would turn it red on a correct lane; and only the no-page-bounds half of the guard has a test, because a mounted app never lacks the shape layer.</p>
<p><b>Judged, round 9 — PASS</b> on the guard and the camera check. A ninth auditor ran the shipped journey against three broken builds (a pinned overlay, a body portal before and after the z-order read) and each went red on the intended assertion; it forced the no-page-bounds branch for one Block and measured the lane at its in-shape placement, and it proved the pan-back restores the camera bit-exactly for the later fixed-coordinate clicks. Two test-hardening risks it named are closed here: the fallback branch had no regression test (reverting the guard left all 2,345 unit tests and every journey check green), so the journey now stubs the page bounds for the Block, opens the lane and asserts its in-shape placement; and the pan check's fixed 50 px magnitude gate would have gone red at zoom ≤ ⅓ on a correct lane, so it now expects the Block to move by the pan × zoom.</p>
<p><b>Judged, round 8 — PASS</b> on the follow-the-move fix. An eighth auditor re-ran the gates, moved the Block twenty times by script with the lane open (caret, selection, focus, CodeMirror history and zero remounts all kept), moved its parent frame, panned the camera ten times (no re-render — the read is page-space), forced the no-shape-layer fallback and measured the lane at its in-shape placement, drove the same in the IDE-host embed with the real bridge, and turned both new assertions red by mutation. Two one-line risks it named are closed here: the style guard now matches the portal guard (no shape layer <i>or</i> no page bounds keeps the in-shape placement, instead of collapsing to the corner when only the bounds were missing); and the journey now also requires the overlay to sit under tldraw's shape layer and pans the camera with the lane open — without that, a portal into <code>body</code> would have passed every check while the lane stopped following the canvas.</p>
