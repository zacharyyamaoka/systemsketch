...
<section class="grid">
  ...
  <article class="card"><h2>Acceptance gates</h2><ul class="clean">{checks_html}</ul></article>
</section>
...
<section class="grid">
  ...
  <article class="card"><h2>Supported seam</h2><p>The saved shape is still <code>type: arrow</code> with ordinary arrow bindings. SystemSketch’s existing <code>ArrowShapeUtil</code> extension paints the captured SVG body and uses sampled points for hit geometry and selection indication.</p><p>Moving a bound Block resolves the authored binding anchors without tldraw’s anti-degeneracy inset. Editing the arrow’s own geometry clears the exact snapshot and returns visual authority to the stock primitive.</p></article>
</section>

<section class="card links"><h2>Run it</h2><p><a href="../sketches/review/detach-edge-fidelity.systemsketch">review fixture</a><a href="../tests/detach_edge_fidelity_smoke.mjs">browser scorer journey</a><a href="build_detach_edge_fidelity.py">gallery builder</a></p><p><code>npm run test:detach-edge-fidelity</code> · <code>npm run check</code></p></section>
