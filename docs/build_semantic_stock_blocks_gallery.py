...
<aside class="facts"><div class="fact"><b>3</b>curated picker presets</div><div class="fact"><b>12</b>real-browser acceptance assertions</div><div class="fact"><b>0</b>new canvas primitives or schedulers</div></aside></header>
...
<article class="panel"><span class="kind">Pure value</span><h2>Select</h2><p>A visible <code>condition bool</code> input row plus <code>true</code> and <code>false</code> candidate values. It is a normal Block, not a Branch region or execution flow feature.</p><p><b>Projection:</b> <code>true_value if condition else false_value</code></p></article>
<article class="panel"><span class="kind">Authoring source</span><h2>Clock / Trigger</h2><p>Typed ordinary <code>Trigger</code> output, persisted source and rate. The derived source/rate declaration is always visible; an optional annotation only appears beneath it when enabled.</p><p><b>Boundary:</b> prototype declares intent; it does not schedule.</p></article></section>
...
<section class="panel" style="margin-top:24px"><h2>Proof surface</h2><table><thead><tr><th>Concern</th><th>Evidence</th></tr></thead><tbody><tr><td>Preset and persistence</td><td><span class="proof">Unit-tested.</span> The named StockConfig migration normalizes legacy values, strips configuration on downgrade, and a malformed current-schema <code>rateHz: 0</code> import is repaired before immediate re-export.</td></tr><tr><td>Clock description contract</td><td><span class="proof">Unit + browser + detach-tested.</span> One effective description drives layout, canvas, SVG export, and detached stock text; the Clock boundary remains visible with no annotation.</td></tr><tr><td>Value/control distinction</td><td><span class="proof">Browser-tested.</span> The actual picker creates Select as a Block with a visible <code>condition</code> row and its inspector shows <code>true_value if condition else false_value</code>.</td></tr><tr><td>Linked Clock</td><td><span class="proof">Browser-tested twice.</span> Positive rate edits repaint the label, propagate through linked occurrences, undo, reload without stale prose, and stay visible but disabled in read-only mode. A real equal-config duplicate/title collision converges; a different 10 Hz collision is visibly <code>Draft 1</code>.</td></tr></tbody></table><div class="boundary"><b>Scoped limitation.</b> Clock / Trigger does not schedule, execute, or make time advance. It persists only authoring source/rate; runtime capability belongs to a separate adapter contract.</div></section>
