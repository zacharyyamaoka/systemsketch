...
<aside class="facts"><div class="fact"><b>3</b>curated picker presets</div><div class="fact"><b>6</b>real-browser acceptance assertions</div><div class="fact"><b>0</b>new canvas primitives or schedulers</div></aside></header>

<section class="grid"><article class="panel"><span class="kind">Record data</span><h2>Set attributes</h2><p>One ordinary <code>record</code> inlet and output, with stable <code>member_N</code> input identities whose editable names are the attributes being changed.</p><p><b>Projection:</b> <code>replace(record, quota=…, enabled=…)</code></p></article>
...
<article class="panel"><span class="kind">Authoring source</span><h2>Clock / Trigger</h2><p>Typed ordinary <code>Trigger</code> output, persisted source and rate. It remains useful before a runtime exists because the inspector says that boundary plainly.</p><p><b>Config:</b> <code>clock · 10 Hz · unavailable</code></p></article></section>
...
<section class="panel" style="margin-top:24px"><h2>Proof surface</h2><table><thead><tr><th>Concern</th><th>Evidence</th></tr></thead><tbody><tr><td>Preset and persistence</td><td><span class="proof">Unit-tested.</span> Factories build the exact rows/config; Clock configuration round-trips as normal Block props.</td></tr><tr><td>Member identity</td><td><span class="proof">Browser-tested.</span> Clicking “Add member update” creates <code>member_2</code>, rather than a generic input id.</td></tr><tr><td>Value/control distinction</td><td><span class="proof">Browser-tested.</span> Select’s input contract is two candidate values plus one <code>bool</code>; its rendered shape remains a Block.</td></tr><tr><td>Runtime honesty</td><td><span class="proof">Browser-tested.</span> Clock exposes source/rate and a visible <em>Runtime adapter unavailable</em> message.</td></tr></tbody></table><div class="boundary"><b>Scoped limitation.</b> Clock / Trigger does not schedule, execute, or make time advance. It is a persisted authoring declaration until a separately implemented runtime-adapter contract exists.</div></section>
