...
.shots {{ grid-template-columns:repeat(2,minmax(0,1fr)) }} figure {{ overflow:hidden; margin:0 }} figure img {{ width:100%; display:block; background:#f6f7f8 }} figcaption {{ padding:14px 16px 17px; color:var(--muted); font-size:14px }} figcaption b {{ color:var(--ink) }}
...
<main>
  ...
  <p class="lede">Both behaviours are opt-in presentation choices. A headed Port or Expanded Block can become a 48px header without severing its external port semantics. An Expanded Block can also fit its actual children after edits, drops, reparenting, and deletes.</p>
  <section class="facts"><div class="fact"><b>3 flags</b><span>folding enabled · folded · auto-fit children</span></div><div class="fact"><b>6 / 6</b><span>real-browser journey checks</span></div><div class="fact"><b>{html.escape(delta)}</b><span>live source/test delta when this gallery was built</span></div></section>
  <section class="card policy"><strong>Membership needs an explicit exit, not a brittle drag race.</strong><br>Right-click an object inside an auto-fit Block and choose <b>Remove from container</b>. The command reparents it to the Block’s parent while preserving its page position; the source Block then refits. This is deliberately clearer than trying to infer an “outside” drag while the boundary is moving beneath the pointer.</section>
  <h2>Three stock-friendly seams</h2>
  <section class="flow"><article class="card step"><h3>Fold the current face</h3><p>The leftmost chevron is visible only on foldable Port and Expanded faces. Folding parks the full dimensions for that view, shows the compact header, and hides descendants from the active frame scope.</p></article><article class="card step"><h3>Fit the real children</h3><p>Expanded Blocks call tldraw’s stock <code>fitFrameToContent</code> after document mutations. Empty Blocks keep their manually chosen starting size.</p></article><article class="card step"><h3>Leave without a fight</h3><p>The context command uses stock <code>reparentShapes</code>, preserving the object’s canvas position. It appears only where an enabled auto-fit container actually owns the selected object.</p></article></section>
  <h2>Observed canvas states</h2>
  <section class="shots"><figure><img src="{expanded}" alt="Expanded SystemSketch Block with folding and auto-fit enabled in its Behaviour inspector"><figcaption><b>Open and auto-fit.</b> The chevron takes the furthest-left header slot. The expanded parent has tightened to its one child, while the Behaviour panel confirms both settings are enabled.</figcaption></figure><figure><img src="{folded}" alt="Folded SystemSketch Block as a compact header with its hidden child"><figcaption><b>Folded.</b> The same Block collapses to its header, its child is hidden, and unfolding restores the parked full box rather than inventing a new size.</figcaption></figure></section>
  <div class="mini"><p><b>Why not use a separate custom drag model?</b> The feature keeps tldraw’s stock frame membership and uses the narrow hooks around it. The one novel behavior is a named escape hatch.</p><button id="prior-art" type="button" aria-pressed="false">Show prior-art boundary</button></div>
  <div class="detail" id="prior-art-detail">tldraw already exposes frame fitting and reparenting primitives. SystemSketch uses those supported mechanisms rather than forking frame drag, resize, or membership behavior; the 56px fit padding simply reserves room for its existing Block header.</div>
  <h2>Proof and review</h2>
  <section class="card evidence"><b>Automated proof</b><ul><li><code>npm run test:block-fold-autosize</code>: creates real stock membership, enables both inspector choices, checks exact fold/unfold restore, and invokes the actual context menu.</li><li>Focused model, migration, visibility, auto-fit, and inspector tests cover persisted state and command predicates.</li><li><code>npm run check</code>: TypeScript plus the full Vitest and Python corpus passed after the capture.</li></ul></section>
