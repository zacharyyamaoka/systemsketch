...
<p class=\"lede\">A real right-click journey covers an ordinary Block, a Branch, a For Loop, and a semantic connection. The Loop now offers the same command as the other authored containers; Branch and Loop become fresh stock Frames so their children and bound arrows survive without custom renderers.</p>
<div class=\"note\"><strong>How to read these pixels.</strong> The browser captured the same canvas rectangle immediately before and after the actual context-menu action. The deterministic heat map measures visual continuity; it is evidence, not a claim that a stock Frame is pixel-identical to a custom authored surface. The requested <code>/llm-judge</code> command was not available in this workspace.</div>
