...
ISSUES = [
    ...
    },
    {
        ...
        "priority": "P1",
        "kind": "Tooling",
        "status": "Open" if missing_journeys else "Implemented",
        "title": "Three scripted diff journeys are committed and runnable" if not missing_journeys else "Three committed npm scripts point to journey files that are not committed",
        "summary": "Merge reconciliation checkpointed the diff-state, diff-vocabulary, and diff-round2 journeys from the busy main checkout; all three now resolve and complete successfully." if not missing_journeys else "The diff-state, diff-vocabulary, and diff-round2 commands fail with MODULE_NOT_FOUND because their journey files are absent.",
        "images": [("docs/assets/edge-vocabulary-4-dragged.png", "The committed diff journeys now exercise the complete state, vocabulary, and round-two visual surfaces." if not missing_journeys else "The nearest committed edge-vocabulary surface is covered; the newer diff journeys themselves are unavailable in this branch.")],
        "actual": "The three package scripts now resolve to committed browser journeys and pass 8, 9, and 11 checks respectively." if not missing_journeys else "npm run test:diff-states, test:diff-vocabulary, or test:diff-round2 exits before opening a browser.",
        ...
        "repro": "npm run test:diff-states && npm run test:diff-vocabulary && npm run test:diff-round2",
        "proof": "All three commands were driven against isolated scratch boards during merge reconciliation: 28/28 browser checks passed with no console errors." if not missing_journeys else f"Missing paths: {', '.join(missing_journeys)}.",
        "seam": "package.json · tests/block_diff_*_smoke.mjs",
        "acceptance": "Keep all three scripts backed by committed journeys and green against isolated boards.",
    },
]
...
<p class="lede">Audited the latest committed Preview merge snapshot, reconciled it with current <code>main</code>, profiled a 116-shape board, parsed every committed review fixture, and drove the affected UI in real headless Chrome. Fixes were isolated during the audit, then merged after main's concurrent work was checkpointed.</p>
<section class="metrics">
 ...
 <div class="metric"><strong>{measurements['verification']['reviewBoardsParsed']} / {measurements['verification']['reviewBoardsParsed']}</strong><span>review boards parse</span></div>
 <div class="metric"><strong>{open_count} + {unconfirmed}</strong><span>open + unconfirmed</span></div>
</section>
<div class="note"><strong>Scope boundary.</strong> The audit began in an isolated worktree and never touched Zach's real board. During merge reconciliation, the live main checkout was checkpointed before conflicts were resolved; its three diff journeys were then verified against isolated scratch boards.</div>
