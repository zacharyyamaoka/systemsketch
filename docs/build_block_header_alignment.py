...
<p class="lede">Port and Expanded Blocks now expose <b>Header alignment</b> in the inspector. Left preserves the established header. Center places the icon/title identity group on the Block’s actual midpoint while the semantic type remains at the right edge. The existing title-formatting popup is deliberately unchanged.</p>
...
</div><div class="diagram"><div class="lane"><div><b>Mirrored metadata lane</b><small>An invisible layout twin reserves the exact same width as the visible type lane.</small></div></div><div class="arrow">→</div><div class="lane center"><div><b>Icon + title</b><small>The middle track remains centered and truncates safely when the Block becomes narrow.</small></div></div><div class="arrow">→</div><div class="lane"><div><b>Visible metadata lane</b><small>Definition/diff badges and semantic type stay anchored to the right.</small></div></div></div></section>

<section><h2>Ready-to-drive review board</h2><p class="sub">The generated fixture contains the left-aligned interaction target and a centered reference, with stock bound callout arrows. The browser journey first drags the target to prove its cue follows, then drives Left / Center and both header-bearing views.</p><figure><img src="{fixture}" alt="Block header alignment review fixture"><figcaption><b>Human review path.</b> Select <code>parse_frame</code>, use View → Header alignment → Center, then switch it back to Left. Compare it with the prepared centered reference below.</figcaption></figure></section>
