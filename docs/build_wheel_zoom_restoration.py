...
.metrics {{ display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin:30px 0 46px; }}
...
.fixture {{ width:100%; border-radius:12px; border:1px solid var(--line); }}
...
@media (max-width:760px) {{ .metrics,.proof {{ grid-template-columns:1fr; }} main {{ padding-top:34px; }} }}
</style>
</head>
<body>
<main>
  <div class="eyebrow">SystemSketch · interaction restoration · 2026-09-04</div>
  <h1>The wheel changes scale again.</h1>
  <p class="lead">An ordinary vertical wheel gesture now zooms around the pointer instead of moving the board up and down. The contract survives a stale tldraw Trackpad preference and applies to the product, development canvases, and the embedded IDE canvas.</p>

  <section class="metrics" aria-label="Measured browser results">
    <div class="metric"><b>{html.escape(before['zoomLabel'])} → {html.escape(after['zoomLabel'])}</b><span>visible zoom readout</span></div>
    <div class="metric"><b>{before['camera']['z']:.2f} → {after['camera']['z']:.2f}</b><span>stock camera scale</span></div>
  ...
  <section class="panel">
    ...
    <p>Drag the slider to compare the actual browser frame immediately before and after a plain wheel-up event. The rectangle expands around the pointer; the bottom-right readout moves from 100% to 110%.</p>
  ...
  <section class="proof">
    <article class="panel">
      ...
      <ul>{checks}</ul>
      <p class="stock">The repair stays on tldraw’s supported seams: <code>options.camera.wheelBehavior</code>, <code>Editor.setCameraOptions</code>, and the public input-mode preference. No wheel listener, camera reimplementation, or tldraw fork was added.</p>
    </article>
    <article class="panel">
      ...
      <p>Place the pointer over <code>wheel_target()</code> and scroll without Ctrl. Pass when the Block grows and shrinks around that point and the visible percentage changes.</p>
    </article>
  </section>
