...
def main() -> None:
    ...
    assert "Normal scroll pans; Ctrl/Cmd + scroll zooms" in recipe
    ...
    modifier_capture = data_uri(ASSETS / "canvas-navigation-modifier-zoom.png", "image/png")
...
<p class="lead">Normal wheel movement now pans the page. Hold <code>Ctrl</code> or <code>⌘</code> while scrolling to zoom. The alternate direct-wheel gesture remains one clear, saved opt-in under <code>Settings → Canvas</code>.</p>
<section class="hero"><video controls autoplay muted loop playsinline poster="{stock_capture}"><source src="{hero_mp4}" type="video/mp4"><img class="fallback" src="{hero_gif}" alt="Actual canvas-navigation journey: stock pan, the Settings switch, direct zoom, and restored stock mode"></video><div class="caption">Actual browser journey · stock pan → settings opt-in → direct wheel zoom → restored stock navigation. A GIF fallback is embedded for browsers without video.</div></section>
<section class="facts"><div class="fact"><b>{len(checks)}/{len(checks)}</b><span>real-browser checks passed</span></div><div class="fact"><b>{stock['camera']['z']:.0%}</b><span>untouched first-run scale</span></div><div class="fact"><b>one switch</b><span>direct zoom remains reachable, not imposed</span></div></section>
<section class="flow"><div class="node"><b>Normal (default)</b>Wheel changes the camera position.</div><div class="arrow">→</div><div class="node"><b>Modifier zoom</b><code>Ctrl/⌘ + wheel</code> changes only scale.</div><div class="arrow">→</div><div class="node"><b>Direct (opt-in)</b>Plain wheel zooms after one Canvas setting.</div></section>
<section class="grid"><article class="panel"><h2>The setting says the whole contract</h2><p>The default does not make people infer the modifier: it spells out both gestures directly, then removes direction and sensitivity controls until direct zoom is deliberately enabled.</p><figure><img src="{default_setting}" alt="Canvas settings with Direct wheel zoom turned off"><figcaption><strong>Default.</strong> Scroll pans; <code>Ctrl/Cmd + scroll</code> zooms.</figcaption></figure></article><article class="panel"><h2>Direct mode stays available</h2><p>Turning on the existing row changes the live tldraw camera immediately and reveals its existing direction and sensitivity choices. Turning it off restores stock panning and the standard zoom gain.</p><figure><img src="{direct_setting}" alt="Canvas settings with Direct wheel zoom turned on"><figcaption><strong>Opt-in.</strong> Direct scroll zoom is enabled; direction and 50–150% sensitivity appear below it.</figcaption></figure></article></section>
<section class="panel" style="margin-top:16px"><h2>Observed camera behavior</h2><table><thead><tr><th>Gesture</th><th>Observed change</th><th>What stays stable</th></tr></thead><tbody><tr><td>Plain scroll</td><td><code>y {stock['camera']['y']:.0f} → {panned['camera']['y']:.0f}</code></td><td>Scale <code>{stock['camera']['z']:.0%}</code></td></tr><tr><td>Ctrl/Cmd + scroll</td><td>Scale <code>{panned['camera']['z']:.0%} → {modifier['camera']['z']:.0%}</code></td><td>Stock pan contract stays selected</td></tr><tr><td>Direct wheel zoom</td><td>Scale <code>{results['directBefore']['camera']['z']:.0%} → {direct['camera']['z']:.0%}</code></td><td>Opt-in remains explicit in local settings</td></tr><tr><td>Disable direct mode</td><td><code>inputMode: {restored['inputMode']}</code></td><td><code>zoomSpeed: {restored['zoomSpeed']:.0f}</code> resets to standard</td></tr></tbody></table><div class="compare-toggle"><span>Stock canvas</span><input id="reveal" type="range" min="0" max="100" value="50" aria-label="Compare stock panning and modifier zoom"><span>Modifier zoom</span></div><div class="compare" style="margin-top:12px"><figure><img id="left" src="{pan_capture}" alt="Canvas after a stock plain scroll pan"><figcaption>Plain scroll: same 100% readout, new page position.</figcaption></figure><div class="swap">⇄</div><figure><img id="right" src="{modifier_capture}" alt="Canvas after Ctrl plus scroll zoom"><figcaption>Ctrl/Cmd + scroll: scale changes to 90%.</figcaption></figure></div></section>
<section class="grid" style="margin-top:16px"><article class="panel"><h2>Human review board</h2><img class="fixture" src="{fixture_capture}" alt="Wheel zoom review fixture with pan, modifier zoom, and direct-wheel setting instructions"><p>Start with the prepared <code>wheel()</code> Block: plain scroll pans, modifier scroll changes the percentage, then the Canvas opt-in changes plain scroll to zoom.</p></article><article class="panel"><h2>Regression proof</h2><ul>{check_items}</ul><p class="quiet">The implementation uses tldraw’s supported <code>camera.wheelBehavior</code>, <code>setCameraOptions</code>, and public <code>inputMode</code> preference. It installs no replacement camera or custom wheel-event handler.</p></article></section>
