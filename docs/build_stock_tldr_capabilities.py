...
<p class="lede">For detach, stock only means the saved result contains ordinary tldraw records—<code>arrow</code>, <code>geo</code>, <code>group</code>, <code>line</code>, and <code>text</code>—with documented stock properties. No SystemSketch shape utility, binding, component, or custom paint layer is needed to open and render it. It does <em>not</em> forbid stock text scale, rich text, curved arrows, or a stock Line with as many persisted polyline points as the route needs.</p>
<div class="callout"><strong>Verified in a bare stock viewer, not inferred from a screenshot.</strong> The 50-case stress proof serializes the detached board to <code>.tldr</code>, then opens it in a route that imports only default <code>createTLSchema</code> and bare <code>&lt;Tldraw /&gt;</code>. It renders 113 stock Lines—including ten authored multi-elbows—with no custom shapes or error boundary. The smaller primitive test also asserts a 36px heading, 18px Branch title, 11px active label, and curved Arrow.</div>
...
<section class="card yes"><span class="chip">SUPPORTED</span><h3>Curves, one-elbows, and authored multi-elbows</h3><p>Stock <code>arrow</code> has <code>kind: 'arc'</code> with numeric <code>bend</code>, and <code>kind: 'elbow'</code>. For a route with several authored bends, detach emits a stock <code>line</code> with every route point persisted in its <code>points</code> record. The bare stock viewer renders that complete polyline; it is not a SystemSketch fallback.</p></section>
...
<tr><th>Frame clipping around a header port</th><td>A stock Frame clips its direct children; a live Region port deliberately straddles its wall.</td><td>Do not use a Frame. Branch and Loop become a movable stock Group containing the card, label, rule, and port geos, so the full 18px rings render in bare stock tldraw.</td></tr>
<tr><th>One dynamically reroutable Arrow with arbitrary manual bends</th><td>A stock Arrow persists endpoints plus one elbow midpoint or an arc bend. It does not persist an arbitrary sequence of user-authored elbow waypoints as a single binding-aware Arrow.</td><td>For exact current rendering, preserve the complete route in a stock Line. An invisible stock Arrow retains endpoint bindings for rebuild. Moving endpoint shapes later will not reroute the frozen Line like the semantic SystemSketch connection; that is the explicit editability trade-off.</td></tr>
<tr><th>SystemSketch port identity and temporal semantics</th><td>Stock shapes have no SystemSketch port id, temporal type, or custom router policy.</td><td>Preserve stock geometry and dash convention. Optional ignored metadata exists only for SystemSketch’s rebuild path; bare tldraw ignores it and still renders the drawing.</td></tr>
