...
| UI | `src/recorder/RecorderControls.tsx`, `recorder.css` | `RecorderControls` (Dev-menu rows between *Isolated presets* and *Version & updates*; `compact` form in the presets' identity bar) and `RecorderIndicator` (the REC bar, **portaled to `document.body`** because the canvas layer sits under tldraw's top chrome). There is deliberately no prefatory text input: add any context in the chat before pasting the packet. Test ids: `recorder-controls` (`data-mode`, `data-enabled`), `recorder-indicator`, `recorder-last-path`; actions `data-action="save-last" | "take" | "copy-last" | "toggle"`, chips `data-window="<ms>"`. |
