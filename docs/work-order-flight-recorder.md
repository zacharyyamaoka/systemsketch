...
| UI | `src/recorder/RecorderControls.tsx`, `recorder.css` | `RecorderControls` is a split capture between *Isolated presets* and *Version & updates*: one broad Save-last face at rest, with Record next, Copy last, duration, and power behind its chevron; during a take the full control becomes Stop and save. The presets use the same interaction in `compact` form. `RecorderIndicator` is the REC bar, **portaled to `document.body`** because the canvas layer sits under tldraw's top chrome. There is deliberately no prefatory text input: add context in the chat before pasting. Test ids: `recorder-controls` (`data-mode`, `data-enabled`), `recorder-status`, `recorder-split`, `recorder-menu`, `recorder-indicator`, `recorder-last-path`; actions `data-action="save-last" | "more" | "take" | "copy-last" | "toggle"`, chips `data-window="<ms>"`. |
...
| Proof | `tests/recorder_smoke.mjs`, `tests/test_recording_store.py`, `src/recorder/flightRecorder.test.ts` | Journey writes `docs/assets/recorder-acceptance.json`, `recorder-sample-packet.txt`, `recorder-sample-timeline.jsonl` and five captures, last, only if all checks pass. |
