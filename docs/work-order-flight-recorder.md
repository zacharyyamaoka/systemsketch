...
| Sidecar | `scripts/recorder_frames.mjs` | `--cdp-port --url-prefix [--window-ms 60000] [--every-nth 2] [--quality 70] [--max-width 0]`. Polls `/json/list`, attaches to every page under the prefix, `Page.startScreencast` only while a take is armed, keeps a 60 s ring per page, and re-arms after navigation. Ops: `arm`, `status`, `dump {dir, fromWall, toWall, keepGapMs, url}`, `quit`. Frames are named by ms since the recording's `startedAtWall`. Node's built-in WebSocket connects to a Chrome started **without** `--remote-allow-origins` (verified). |
