...
<h2>Real review board</h2><figure><img alt="SystemSketch review board with an Event Block, a Control-named Branch dot, and a legal connecting cable" src="data:image/png;base64,{image}"><figcaption>The generated board contains a real Block, a real Branch control endpoint, a real bound cable, and bound instructional arrows. It was cold-reopened through the application serializer before capture.</figcaption></figure>
...
<p class="warning"><strong>Review gesture:</strong> select <code>emit_tick()</code>, inspect the Event selector on <code>tick</code>; then select the cable. Pass when its inspector names the Branch endpoint, shows provenance plus the mismatch warning, and the Branch dot visibly and accessibly says <code>Control</code>.</p>
<p class="quiet">Audit repairs: provenance-only inspector changes now refresh; Value-pill normalisation preserves terminal-local claims; malformed or half-bound cables state that no role was inferred; Branch and Loop endpoint names are readable; semantic dot labels are real DOM text. Evidence: focused unit tests plus a real-app fixture cold reopen and browser journey.</p>
