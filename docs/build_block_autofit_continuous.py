...
def build() -> str:
    ...
    child_first = png(ROOT / "docs" / "assets" / "block-autofit-child-first-drag-2026-09-05.png")
...
<section class="cards"><article class="card"><b>Stock gesture</b><span>Translation remains the sole persisted geometry writer while the pointer is held.</span></article><article class="card"><b>Paint-only preview</b><span>The React Block surface follows current padded child bounds without entering tldraw’s cached geometry graph.</span></article><article class="card"><b>Stock fit</b><span>Pointer-up commits the identical projected box once, preserving page pose and membership.</span></article></section>
<h2>The supported-seam pipeline</h2><section class="flow"><article class="phase"><small>01 · MOVE</small><b>tldraw updates the child</b><span>Every pointer sample is still calculated from the engine’s original transform snapshot.</span></article><div class="arrow">→</div><article class="phase"><small>02 · PAINT</small><b>BlockCanvas previews the fit</b><span>The React presentation reads padded child bounds. <code>ShapeUtil.getGeometry</code> stays pure and record-backed.</span></article><div class="arrow">→</div><article class="phase"><small>03 · RELEASE</small><b>tldraw fits once</b><span>The public frame helper replaces the preview with persisted frame and child-local coordinates.</span></article></section>
...
<h2>Review surfaces</h2><section class="shots"><figure><img src="{child_first}" alt="A nested Block selected during the first drag after its parent was selected"><figcaption><b>First-drag ownership.</b> Click the outer header, then immediately drag <code>parse()</code>. The child becomes the stock translation target before the gesture snapshots selection.</figcaption></figure><figure><img src="{fixture}" alt="Continuous Block auto-fit fixture with numbered instructions"><figcaption><b>Continuous auto-fit board.</b> The nested <code>parse()</code> Block is ready for rapid edge-crossing drags. The orange steps and green pass card describe only visible behavior.</figcaption></figure><figure><img src="{fold}" alt="Fold-control review board showing left and right placements"><figcaption><b>Header composition update.</b> Left remains the fold default. With the chevron at far right, the type label joins the title on the left instead of competing for the corner.</figcaption></figure></section>
<h2>Evidence</h2><section class="proofs"><article class="panel"><h3>Real-browser gates</h3><ul><li><code>test:block-autofit-continuous-fixture</code>: parent-then-child first drag, 32 held edge crossings, 50 rapid stress samples, zero held parent writes, retained membership and no crash surface.</li><li><code>test:block-fold-autosize</code>: fold, unfold, stable drag, and explicit Remove from container behavior.</li><li><code>test:block-fold-control-side-fixture</code>: Left/Right persistence, both view modes, and title-to-type adjacency.</li><li><code>test:rows</code>: latest-main heading rows, branch rows, menus, and inspector drag remain intact.</li></ul><div class="numbers"><div class="metric"><b>82</b><span>held samples</span></div><div class="metric"><b>0</b><span>held parent writes</span></div><div class="metric"><b>0</b><span>detachments/errors</span></div></div></article><article class="panel why"><h3>What caused the stack overflow?</h3><p>The old <code>getGeometry</code> implementation read live SelectTool state and descendant cached geometry while tldraw was computing the parent’s own cached geometry. That made the reactive parents depend back on themselves. Geometry is record-only again; continuous feedback is a paint-only projection, and <code>fitFrameToContent</code> remains the correct one-shot release operation.</p><p style="margin-top:15px"><b>Latest-main reconciliation.</b> Current <code>main</code> through <code>{main_base}</code> is merged into this branch, retaining the newer row/heading and Code Block work.</p></article></section>
