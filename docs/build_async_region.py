...
<section><p class="eyebrow">Driven carrier-selection journey</p><h2>Choose the meaning; keep the graph.</h2><p class="intro">This 9.7-second viewport recording is the real stress board: switch to Components, choose Service response and Action result, choose Shortest for both, then focus A2. The component positions and 18 canonical connection records remain unchanged throughout.</p><div class="hero-media"><video autoplay muted loop playsinline controls poster="{data_uri(STRESS_ASSETS / '02b-shortest-tracks.png')}"><source src="{data_uri(STRESS_ASSETS / 'representative-edge-hero.mp4', 'video/mp4')}" type="video/mp4"><img class="hero-gif" src="{data_uri(STRESS_ASSETS / 'representative-edge-hero.gif', 'image/gif')}" alt="Animated fallback of the representative-edge selector journey"></video><div class="hero-note"><b>REAL CHROME SCREENCAST · 9.7 S · MUTED LOOP</b><span>Response / Result → independently measured Shortest → A2 focus</span></div></div></section>
