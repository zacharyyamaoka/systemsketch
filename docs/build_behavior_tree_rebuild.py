...
CAPTURES = [
    ...
    ("tree-wires-straight.png", "Tree wires · straight", "Parent exit to child entry, no bend — the plainest of the four styles."),
    ("tree-wires-elbow.png", "Tree wires · elbow", "A right-angled bus along the reading axis, the default (and the only style before this pass)."),
    ("tree-wires-curved.png", "Tree wires · curved", "A cubic Bézier whose control points sit on the reading axis, halfway between parent and child — the same 4-point shape `edgePathData` draws with an SVG `C` command."),
    ("tree-wires-slanted.png", "Tree wires · slanted", "A single diagonal stub off the parent's exit face, straight into the child — no right angle, no curve."),
