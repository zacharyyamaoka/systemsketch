...
<style>
  ...
  * {{ box-sizing:border-box }} body {{ margin:0; background:radial-gradient(circle at 15% 0,#1e2f48,transparent 36%),var(--bg); color:var(--ink); font:15px/1.55 Inter,ui-sans-serif,system-ui,sans-serif }}
  ...
  <section class="grid">
    ...
    <article class="card"><h2>Migration + proof</h2><ul><li>8 model regressions: future/malformed protection, independent undo marks, read-only refusal, no-op rename, and exact transformed multi-page merge.</li><li>9 real-browser saved-view checks plus the 24-step two-page reopen journey. Secondary landmarks merge with stable collision names; unrelated deleted-page metadata stays on each imported Frame.</li><li>The review board is a focused, editable fixture with a visible return-to-view pass condition.</li></ul></article>
