...
FIXTURE = ROOT / "sketches" / "review" / "floating-port-direct-drag.png"
...
<p class="lede">A tiny circular endpoint can sit anywhere on the canvas, carry a typed value, and wire directly into or out of a Block. It keeps the useful semantics of a Block port without requiring a containing card—and its visible label is now the one-press handle for moving the whole primitive.</p>
<div class="chips"><span class="chip ok">7/7 live browser checks</span><span class="chip">Port → Block → Port</span><span class="chip">first-grab label move</span><span class="chip">derived or explicit fill</span><span class="chip">stock tldraw seams</span></div>
<section><h2>Live proof</h2><p class="sub">This real product capture shows a free output Port wired into <code>integrate()</code>, a Block output wired back to a free input Port, and the selected source’s compact inspector. The input Port was moved by dragging its visible text once; its real cable followed.</p><figure><img src="{live}" alt="SystemSketch canvas with two free Ports wired through a Block and a Port inspector"><figcaption><b>Actual browser canvas.</b> The source Port is set to <code>speed · float = 7.2</code>, Output, Offset, and Auto. The moved <code>state · Number</code> Port still owns its cable; the circular dot remains a dedicated wire handle.</figcaption></figure></section>
...
<section><h2>Review board</h2><p class="sub">The focused disposable board starts with a real Port-to-Block cable and asks for the one gesture that regressed: press directly on the label and move. Its orange cue stays bound to the Port while it moves.</p><figure><img src="{fixture}" alt="Floating Port review board with a direct-drag instruction and green pass card"><figcaption><b>Human review fixture.</b> It opens as an ordinary <code>.systemsketch</code> board, not a hand-drawn mockup.</figcaption></figure></section>
<section><h2>Deliberate boundary</h2><div class="boundary"><b>Free placement wins for now.</b> The original idea considered snapping a Port to primitive edges, then correctly withdrew it: a Port is the compositional atom, so it stays exactly where its author drops it until the app has one shared edge-snapping grammar. Its semantic geometry makes both the dot and label real hit areas, then lets tldraw run its stock translating interaction rather than recreating a movement system; portable export still lowers the shape to ordinary primitives.</div></section>
<footer>Evidence: <code>npm run test:floating-port</code> (7 live checks), <code>npm run test:export</code> (11 portable-export checks), the cold-reopened review-fixture generation, and a real saved-fixture drive. Source: <code>src/floatingPort/</code>, <code>src/blocks/connections/blockPorts.ts</code>, and <code>sketches/review/floating-port-direct-drag.systemsketch</code>.</footer>
</main></body></html>"""
