...
def node_svg(state: dict, label: str) -> str:
    ...
    for index, node in enumerate(nodes):
        ...
        node_markup.append(
            ...
            f'font-family="ui-monospace, SFMono-Regular, monospace">N{index + 1}</text></g>'
        )
    return (
        f'<svg viewBox="{x:.2f} {y:.2f} {width:.2f} {height:.2f}" '
        f'role="img" aria-label="{esc(label)}" preserveAspectRatio="xMidYMid meet">'
...
def edge_card(item: dict) -> str:
    before = item["before"]
    after = item["after"]
    if after["metrics"]["forcedCrossings"]:
        verdict, tone = "TOPOLOGY", "warn"
    elif after["metrics"]["overlap"]:
...
def node_card(item: dict) -> str:
    ...
    <article class="case" data-kind="nodes" data-difficulty="{item['difficulty']}">
      ...
      <button class="compare" type="button" aria-label="Expand {esc(item['title'])} comparison">
        <figure><figcaption><b>Before</b><span>{len(before['nodes'])} nodes · {len(before['edges'])} edges</span></figcaption>
      ...
      <p class="expect"><b>Observed:</b> no node overlap; anchored at the original top-left; existing elbow router owns every shown cable.</p>
    </article>'''


def build(data: dict) -> str:
    summary = data["summary"]
    stress = data["nodeCases"][-1]
...
<title>Tidy edges + Organize nodes — 40-case evaluation</title>
<style>
  ...
  * {{ box-sizing:border-box }} html {{ background:var(--bg); color:var(--ink); font-family:Inter,ui-sans-serif,system-ui,sans-serif }}
  ...
  .kpis {{ display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:12px; margin:34px 0 24px }}
  ...
  @media (max-width:1000px) {{ .kpis {{ grid-template-columns:repeat(2,1fr) }} .cases {{ grid-template-columns:1fr }} }}
  ...
  <p class="lede">Forty synthetic cases, ordered from the smallest defect to a 50-node / 80-edge stress board. Every one is rendered twice from the code being shipped: before, then after.</p>
  <section class="kpis" aria-label="evaluation summary">
    <div class="kpi"><b>20 + 20</b><small>edge and node cases</small></div>
    ...
    <div class="kpi"><b>{stress['layoutMs']} ms</b><small>50 nodes / 80 edges, warm ELK run</small></div>
  </section>
  <section class="contract">
    <article><h2>Tidy edges = preserve the board</h2><p>Moves only shared interior rails. Endpoints stay welded; nodes stay put; authored elbow routes are immovable constraints; curved and straight edges are ignored. The 14-unit pass is deterministic and undoable.</p></article>
    <article><h2>Organize nodes = choose new positions</h2><p>Moves Blocks through a one-shot, selection-first ELK layout. It preserves reading/model order when free, anchors the result where the input began, and takes no edge geometry from ELK—the stock SystemSketch elbow router redraws it.</p></article>
  </section>
  <nav class="toolbar" aria-label="comparison filters">
    <button data-filter="all" aria-pressed="true">All 40</button>
    <button data-filter="edges" aria-pressed="false">Tidy edges · 20</button>
    <button data-filter="nodes" aria-pressed="false">Organize nodes · 20</button>
  ...
  <section class="method">
    ...
    <div class="method-grid">
      <div><p><b>Edge path:</b> each fixture feeds the shipped pure <code>nudgeRoutes</code> implementation. The report validates orthogonality, fixed endpoints, forced-crossing classification, immutable authored routes, and before/after coincident-run length.</p>
      <p><b>Node path:</b> each fixture feeds the shipped <code>organizeGraph</code> wrapper and the exact PyBlocks/Dify-derived ELK option block. The output is run twice byte-for-byte to assert determinism, then every cable shown is recomputed with SystemSketch’s own <code>routeElbow</code>.</p></div>
      ...
      </ul><p>Pink <b>CONSTRAINED</b> cases retain geometry the contract says not to move. Amber <b>TOPOLOGY</b> cases contain a crossing no one-dimensional channel order can remove. Those are surfaced, not hidden.</p></div>
    </div>
  </section>
</main>
