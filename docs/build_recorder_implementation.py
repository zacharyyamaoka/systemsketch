...
<section>
  ...
  <ol class="steps">
    ...
    <li><b>Dev menu, REC bar, policy — <code>src/recorder/RecorderControls.tsx</code>, <code>recorderStore.ts</code></b><span>The Recording section sits between Isolated presets and Version &amp; updates, in the panel's own row grammar; window chips 5 / 15 / 30 / 60 / 120 s; a note field whose text becomes the first line of the packet; an On/Off chip. The bar is portaled onto <code>document.body</code> because the canvas layer sits under tldraw's top chrome. The presets get the same three verbs in a compact strip inside their identity bar.</span></li>
...
<section>
  ...
  <p class="sub">One fresh headless instance: draw two Blocks, wire them, press Escape, save the last 30 s from the Dev menu with a note; then a 5 s take that stops itself; then open the folder's <code>playback.html</code>; then turn the recorder off and reload. Everything below is read from that run.</p>
  <div class="two">
    <figure><img src="__SHOT_MENU__" alt="The Dev menu with the Recording section"><figcaption><strong>The Recording section, in the Dev menu</strong> Save the last 30 s · Record next ≤ 30 s · Copy last recording · the note field · window chips · On.</figcaption></figure>
...
<section>
  ...
  <div class="two">
    ...
    <div class="card">
      <h3>Every time</h3>
      <p>Something odd happens → <code>&lt;/&gt;</code> → <b>Save the last 30 s</b> (type a line into <i>What went wrong?</i> first if you like) → paste into the agent. The paste is prose plus absolute paths; the agent Reads <code>README.md</code>, greps <code>timeline.jsonl</code>, opens the frames it needs. <b>Copy last recording</b> refills the clipboard after a reload. For a bug you can reproduce, <b>Record next ≤ 30 s</b>, do it, stop or let the cap stop it. Open any folder's <code>playback.html</code> to watch it yourself.</p>
    </div>
  </div>
</section>
...
<section>
  ...
  <ul>
    ...
    <li><b>No merge.</b> The work is on <code>__BRANCH__</code> at <code>__HEAD__</code>, with main merged in, pushed to origin. From your checkout: <code>git merge --ff-only __BRANCH__</code>.</li>
  </ul>
</section>
