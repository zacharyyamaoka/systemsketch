...
<section class="section"><h2>Same edge, four useful views</h2><p>Tunnel is presentation state, not a second line primitive. Hovering the invisible middle, selecting an endpoint or the edge, reattaching a terminal, or focusing its named layer restores the complete existing route.</p><div class="switcher" role="group" aria-label="Tunnel evidence frame"><button aria-pressed="true" data-frame="hidden">Idle tunnel</button><button aria-pressed="false" data-frame="focused">Layer focused</button><button aria-pressed="false" data-frame="inspector">Inspector</button><button aria-pressed="false" data-frame="review">Review board</button></div><figure class="frame"><img id="evidence" src="__HIDDEN__" width="__HIDDEN_W__" height="__HIDDEN_H__" alt="Idle edge tunnel with two short stubs and via dots"><figcaption id="caption">Idle: the middle run is visually underground while the real edge remains available to hit testing.</figcaption></figure></section>

<section class="section"><h2>The reveal contract</h2><div class="flow"><div><b>Tunnel toggle</b><span>Stored on the ordinary connection</span></div><div><b>Named layer</b><span>Reusable names derived from tunneled edges</span></div><div><b>View-local focus</b><span>Does not dirty or rewrite the board</span></div><div><b>Reveal context</b><span>Edge · endpoint · hover · reattach · layer</span></div><div><b>One routed path</b><span>Canvas, export, hit target, and autosave agree</span></div></div></section>
...
const frames={hidden:{src:'__HIDDEN__',w:__HIDDEN_W__,h:__HIDDEN_H__,alt:'Idle edge tunnel with two short stubs and via dots',caption:'Idle: the middle run is visually underground while the real edge remains available to hit testing.'},focused:{src:'__FOCUSED__',w:__FOCUSED_W__,h:__FOCUSED_H__,alt:'Complete route revealed by the Diagnostics layer focus chip',caption:'Layer focus: every tunnel assigned to Diagnostics returns as a complete route.'},inspector:{src:'__INSPECTOR__',w:__INSPECTOR_W__,h:__INSPECTOR_H__,alt:'Visibility inspector with Tunnel toggle and layer chooser',caption:'The selected-connection inspector stores the tunnel switch and a reusable layer name.'},review:{src:'__REVIEW__',w:__REVIEW_W__,h:__REVIEW_H__,alt:'Numbered edge tunnel review board',caption:'The generated fixture gives three real gestures and one visible pass condition.'}};document.querySelectorAll('[data-frame]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-frame]').forEach(other=>other.setAttribute('aria-pressed',String(other===button)));const frame=frames[button.dataset.frame],image=document.querySelector('#evidence');image.src=frame.src;image.width=frame.w;image.height=frame.h;image.alt=frame.alt;document.querySelector('#caption').textContent=frame.caption}))
</script></body></html>'''
