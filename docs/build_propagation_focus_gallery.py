...
clear: exactly the original board, byte-for-byte</pre></div><h2>Verification</h2><ul><li>Pure graph-walk coverage proves asymmetric bounds, fan-in/fan-out, zero-step behavior, and cycles that retain their closing cable without looping.</li><li>Real browser journey: select <code>join()</code>, activate Focus flow, set downstream to 2, clear; controls expose accessible names and keyboard shortcuts <code>F</code> / <code>Escape</code>.</li><li>The review fixture was authored through the real editor, cold-reopened, containment-checked, and its saved SHA-256 stayed <code>1e8b16d23fb350fb1000201de4e5ec810338d7c7d1be757be26fefe38306363e</code> before and after focus interaction.</li></ul><p class="quiet">Review artifact: <code>sketches/review/propagation-focus.systemsketch</code>. This branch intentionally does not merge itself.</p></body></html>'''
