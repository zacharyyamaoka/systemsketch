...
def main() -> None:
    facts = measured()
    assert facts["lead_reuses"] and facts["marker_orient"] and facts["inspector_only"]
...
<p class="lead">Slanted is an uncommon ordinary-arrow route: it leaves the source horizontally, then reaches its destination in one diagonal. It lives only in the Inspector, keeps the everyday arrow menu small, and stays a valid stock tldraw arrow when opened elsewhere.</p>
...
<section class="hero"><img src="{data_uri(CANVAS_SHOT)}" alt="SystemSketch with a selected slanted violet arrow that first goes horizontally and then diagonally upward"><div class="caption"><strong>Actual browser render.</strong> The final segment is diagonal and its open marker automatically follows that segment; the right-side Inspector shows Slanted as the active choice.</div></section>
<div class="split"><article class="shot"><img src="{data_uri(INSPECTOR_SHOT)}" alt="Arrow routing controls in the SystemSketch Inspector showing the slanted arrow icon"><p><strong>Quiet control surface.</strong> The reference icon is rendered as one horizontal leg plus one rising diagonal; activation uses the current theme’s accent rather than a hard-coded purple.</p></article><article class="shot"><img src="{data_uri(FIXTURE_SHOT)}" alt="Saved review board with a violet slanted behavior branch, orange steps, and a green pass card"><p><strong>Saved human-review board.</strong> A real bound arrow joins <em>condition?</em> to <em>run action</em>; the orange cues and green pass condition remain editable canvas objects.</p></article></div>
...
<section style="margin-top:42px"><div class="eyebrow">One narrow dataflow</div><div class="flow"><div class="step"><b>1 · Select</b><span>An ordinary tldraw arrow opens the facts Inspector with no new toolbar preset.</span></div><div class="step"><b>2 · Set Slanted</b><span>A versioned metadata flag is added while the stock fallback becomes a zero-bend arrow.</span></div><div class="step"><b>3 · Render</b><span>The existing lead helper yields the horizontal run; one SVG line reaches the terminal.</span></div><div class="step"><b>4 · Revert</b><span>Straight tombstones the metadata and returns the same arrow to the stock renderer.</span></div></div></section>
