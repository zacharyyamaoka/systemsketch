...
DIRECTIONS_SHOT = ASSETS / "slanted-arrow-directions-live-2026-09-04.png"
...
def measured() -> dict[str, object]:
    ...
    return {
        ...
        "lead_reuses": "getConnectionControlPoints({ x: 0, y: 0 }, { x: span, y: 0 })" in source,
...
<p class="lead">Slanted is an uncommon ordinary-arrow route: it leaves a bound source on that face’s normal axis, then reaches its destination in one diagonal. Loose arrows use the dominant endpoint axis. It lives only in the Inspector, keeps the everyday arrow menu small, and stays a valid stock tldraw arrow when opened elsewhere. Its elbow begins at the established automatic lead and becomes an authored control only when dragged.</p>
<section class="decision"><div class="eyebrow">Decision from prior art</div><p><strong>Use React Flow’s explicit source direction and ELK’s short-then-sloped reading, not either engine’s full layout pass.</strong> React Flow exposes <code>sourcePosition</code> specifically so a route can leave the correct side; ELK’s polyline sloped-edge zone preserves a readable short run before the slope. A freeform SystemSketch arrow must not let layout relocate authored endpoints, so it reads the existing binding face and reuses the established connection-lead scalar; SVG’s native <code>orient=&quot;auto&quot;</code> supplies the diagonal arrowhead orientation.</p></section>
...
<section class="hero"><img src="{data_uri(CANVAS_SHOT)}" alt="SystemSketch with a selected heavy-stroke Slanted arrow that leaves rightward before rising diagonally"><div class="caption"><strong>Actual XL browser render.</strong> The final segment is diagonal and one padded native marker follows it. The stock fallback head is suppressed only for Slanted, preventing two mismatched tangents from painting as a malformed heavy-stroke head.</div></section>
<section class="hero"><img src="{data_uri(DIRECTIONS_SHOT)}" alt="Real SystemSketch canvas matrix of eight loose Slanted arrows and four arrows bound to right, bottom, left, and top source faces"><div class="caption"><strong>Direction matrix from the real browser.</strong> Eight loose diagonal directions choose a deterministic dominant axis; four bound arrows instead leave their right, bottom, left, or top source face directly.</div></section>
<div class="split"><article class="shot"><img src="{data_uri(INSPECTOR_SHOT)}" alt="Arrow routing controls in the SystemSketch Inspector showing the slanted arrow icon"><p><strong>Quiet control surface.</strong> The reference icon remains one short horizontal leg plus a rising diagonal; activation uses the current theme’s accent rather than a hard-coded purple.</p></article><article class="shot"><img src="{data_uri(FIXTURE_SHOT)}" alt="Saved review board with a violet Slanted behavior branch exiting down from a condition, orange steps, and a green pass card"><p><strong>Saved human-review board.</strong> Select the real bottom-bound arrow, drag its round elbow dot up or down, then switch away and back to Slanted to restore the untouched automatic lead.</p></article></div>
<section style="margin-top:42px"><div class="eyebrow">What was borrowed, what was not</div><div class="grid"><article class="card prior"><h2>React Flow source positions</h2><p><a href="https://reactflow.dev/api-reference/utils/get-smooth-step-path">React Flow’s SmoothStep utility</a> accepts explicit <code>sourcePosition</code> and <code>targetPosition</code>. That is the key contract: a branch should leave the port or source face that actually owns it.</p><p><strong>Borrowed:</strong> direction-aware departure semantics.</p></article><article class="card prior"><h2>ELK polyline routing</h2><p><a href="https://eclipse.dev/elk/reference/options/org-eclipse-elk-layered-edgeRouting-polyline-slopedEdgeZoneWidth.html">ELK’s sloped-edge-zone option</a> preserves readable polyline runs while allowing a slope where one is needed. <a href="https://eclipse.dev/elk/reference/options/org-eclipse-elk-edgeRouting.html">POLYLINE</a> is an official routing mode.</p><p><strong>Borrowed:</strong> the short-run-then-slope visual grammar.</p></article><article class="card boundary"><h2>Why no new router</h2><p>ELK’s layout algorithms own node placement; calling them for one manually drawn arrow would change the whiteboard’s contract. libavoid is a separate obstacle-router runtime, not a dependency needed for this two-segment route.</p><p><strong>Kept:</strong> authored endpoints and stock binding behavior.</p></article><article class="card proof"><h2>Stock fallback</h2><p>The custom route is a versioned namespaced <code>meta</code> enhancement on an otherwise valid, zero-bend stock arrow. Switching back to Straight writes a JSON tombstone because tldraw merges nested metadata patches.</p><p><strong>Result:</strong> no forked tldraw arrow schema and no stale custom route.</p></article></div></section>
<section style="margin-top:42px"><div class="eyebrow">One narrow dataflow</div><div class="flow"><div class="step"><b>1 · Select</b><span>An ordinary tldraw arrow opens the facts Inspector with no new toolbar preset.</span></div><div class="step"><b>2 · Set Slanted</b><span>A versioned metadata flag is added while the stock fallback becomes a zero-bend arrow.</span></div><div class="step"><b>3 · Default</b><span>The source binding face, or a loose arrow’s dominant axis, chooses the virtual elbow’s short lead.</span></div><div class="step"><b>4 · Drag</b><span>The dot promotes to a vertex and stores only its relative position along that source axis.</span></div><div class="step"><b>5 · Revert</b><span>Straight tombstones the metadata and returns the same arrow to the stock renderer.</span></div></div></section>
