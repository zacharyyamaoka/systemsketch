...
async function main() {
  ...
  try {
    ...
    assert.equal(initial.defaultPlaceholder, '', "Default's own placeholder is blank — the external '=' already carries that meaning")
    ...
    assert.equal(off.defaultPlaceholder, '=', "Default's own placeholder returns to today's literal '='")
