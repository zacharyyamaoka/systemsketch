...
try {
  ...
  assert.equal(await evaluate(page, 'document.querySelectorAll("[data-figure]").length'), 10)
  ...
  await evaluate(page, `document.querySelector('[data-view="stress-exact-phases"]').click()`)
  await delay(100)
  assert.deepEqual(await evaluate(page, '[...document.querySelectorAll("[data-figure].active")].map((node) => node.dataset.figure)'), ['fixture', 'stress-exact-phases'])
  ...
  process.stdout.write('  PASS  the embedded hero and all ten base/stress evidence views render and switch independently without console errors\n')
} finally {
  page?.close()
  session.kill()
}
